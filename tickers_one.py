...
class DatabaseManager:
    ...
    def generate_instrument_hash(self, row: pd.Series) -> str:
        ...
        exclude_columns = ['created_at', 'updated_at', 'last_seen_date', 'instrument_hash', 'last_price_pull_date']
        
        # Core identifying fields for instruments
        core_columns = ['symbol', 'exchangecode', 'securitytype', 'underlier', 'strike', 'expiration', 'optiontype']
    ...
    def insert_new_records(self, df: pd.DataFrame) -> Tuple[int, int]:
        ...
        try:
            ...
            df['last_seen_date'] = current_date
            df['last_price_pull_date'] = None  # New instruments haven't had price data pulled yet
    ...
    def _insert_chunked(self, df: pd.DataFrame) -> Tuple[int, int]:
        ...
        for i in range(0, len(df), chunk_size):
            ...
            try:
                ...
                chunk['last_seen_date'] = current_date
                chunk['last_price_pull_date'] = None
    ...
    def update_last_price_pull_date(self, symbols: List[str]):
        """Update last_price_pull_date for symbols that had price data pulled."""
        if not symbols:
            return
        ...
        try:
            with self.engine.begin() as connection:
                ...
                connection.execute(text(f"""
                    CREATE TABLE #temp_symbols (symbol VARCHAR(100))
                """))
                ...
                for i in range(0, len(symbols), chunk_size):
                    ...
                    values = ",".join([f"('{sym}')" for sym in chunk])
                    connection.execute(text(f"""
                        INSERT INTO #temp_symbols (symbol) VALUES {values}
                    """))
